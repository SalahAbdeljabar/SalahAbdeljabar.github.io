---
title: "Bridging the Educational Divide: A Delay-Tolerant Networking Approach for Equitable Digital Learning in Rural Areas"
collection: publications
category: manuscripts
permalink: /publication/2025-dtn-educational-divide
excerpt: "DTN framework that delivers digital learning content in rural areas by leveraging transport-based data mules."
date: 2025-10-01
venue: "arXiv"
paperurl: "https://arxiv.org/abs/2511.20334v1"
paperpdf: "/files/publications/dtn-educational-divide.pdf"
citation: "**S. Abdeljabar** and M.-S. Alouini. (2025). \"Bridging the Educational Divide: A Delay-Tolerant Networking Approach for Equitable Digital Learning in Rural Areas.\" <i>arXiv preprint arXiv:2511.20334</i>."
---

## Abstract
Access to quality education remains unequal in rural regions where internet connectivity is scarce. This work proposes a delay-tolerant networking (DTN) framework that uses existing transportation infrastructure as data mules to distribute educational content. The approach provides an affordable and sustainable alternative to always-connected models, with a prototype deployment on a university campus demonstrating feasibility for equitable digital learning.


[[PDF]](/files/publications/2025-10-01-dtn-educational-divide.pdf)
